...
int equalStacks(int h1_count, int* h1, int h2_count, int* h2, int h3_count, int* h3) {
    ...
    // Calculate cumulative sums from the bottom for h1
    for (int i = h1_count - 1; i >= 0; i--) {
        sum1 += h1[i];
        insert(&set1, sum1);
    }
    
    // Calculate cumulative sums from the bottom for h2
    for (int i = h2_count - 1; i >= 0; i--) {
        sum2 += h2[i];
        insert(&set2, sum2);
    }
    
    // Calculate cumulative sums from the bottom for h3
    for (int i = h3_count - 1; i >= 0; i--) {
        sum3 += h3[i];
        insert(&set3, sum3);
    }
    
    // Find the common maximum value in all three sets
    Node* current = set1.head;
    int max_common = 0;
    while (current != NULL) {
        int value = current->data;
        if (contains(&set2, value) && contains(&set3, value)) {
            if (value > max_common) {
                max_common = value;
            }
        }
