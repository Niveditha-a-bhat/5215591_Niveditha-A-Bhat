...
int* maxSubarray(int arr_count, int* arr, int* result_count) {
    ...
    *result_count = 2;
    ...
    // For subsequence: sum all positive numbers, if no positive then take the maximum element
    int max_subseq = arr[0];
    int sum_positive = 0;
    ...
    if (has_positive) {
        max_subseq = sum_positive;
    } else {
        max_subseq = max_element;
    }
