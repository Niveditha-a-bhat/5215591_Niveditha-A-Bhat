...
int* waiter(int number_count, int* number, int q, int* result_count) {
    *result_count = number_count;
    ...
    // Create stack A (array where last element is top)
    int* A = malloc(number_count * sizeof(int));
    int topA = -1;
    
    // Initialize stack A with initial numbers (push in reverse order)
    ...
    for (int i = 0; i < q; i++) {
        ...
        while (topA >= 0) {
            ...
            if (plate % prime == 0) {
                ...
                nextA[++topNextA] = plate; // Push to nextA
            }
        }
        
        // Output B from bottom to top
        for (int j = 0; j <= topB; j++) {
            result[result_index++] = B[j];
        }
        
        // Set A to nextA for next iteration
        free(A);
        free(B);
        A = nextA;
        topA = topNextA;
    ...
    // Output remaining A from bottom to top
