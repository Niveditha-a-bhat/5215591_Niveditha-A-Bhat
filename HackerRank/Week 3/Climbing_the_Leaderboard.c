...
int* climbingLeaderboard(int ranked_count, int* ranked, int player_count, int* player, int* result_count) {
    *result_count = player_count;
    ...
    // Create a dense ranked array (remove duplicates)
    ...
    int j = unique_count - 1; // Start from the lowest unique score
    
    for (int i = 0; i < player_count; i++) {
        int current_score = player[i];
        
        // Move upwards until we find the appropriate position
        while (j >= 0 && current_score >= unique_ranked[j]) {
            j--;
        }
        
        // j+1 is the rank (1-indexed)
        result[i] = j + 2;
    }
