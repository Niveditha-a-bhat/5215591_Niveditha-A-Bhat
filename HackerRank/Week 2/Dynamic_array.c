...
int* dynamicArray(int n, int queries_rows, int queries_columns, int** queries, int* result_count) {
    ...
    // Create n empty arrays (using dynamic allocation)
    int** arr = malloc(n * sizeof(int*));       // array of int*
    int* sizes = calloc(n, sizeof(int));        // to track size of each arr[i]
    int* capacities = calloc(n, sizeof(int));   // to track capacity for realloc

    // Result array for storing answers of type 2 queries
    ...
    *result_count = 0;
    ...
    // Free helper arrays (but not results)
    free(sizes);
    free(capacities);
    for (int i = 0; i < n; i++) {
        free(arr[i]);
